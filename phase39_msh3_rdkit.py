#!/usr/bin/env python3
"""
phase39_msh3_rdkit.py
---------------------
Phase 39: MSH3 ATPase (3THW) docking with proper rdkit ligand prep.

Phase 38 used obabel fallback throughout because numpy 2.x broke rdkit's
C extension (_ARRAY_API not found). This run downgrades numpy to <2 first,
enabling ETKDGv3 + MMFF conformer generation for higher-quality 3D poses.
All other parameters identical to Phase 38.

GCS output: gs://.../aegis_flashoptim/phase39_msh3_rdkit/results.json
"""

# ── Force numpy<2 before any rdkit/torch import ───────────────────────────────
...
GCS_OUT       = f"{GCS_BASE}/phase39_msh3_rdkit"
...
OUT_DIR = Path("/tmp/phase39_msh3_rdkit")
...
def detect_adp_box_center(pdb_path):
    ...
    log(f"WARNING: No ADP/ATP ligand found in chain {CHAIN_ID} — using literature fallback")
...
def smiles_to_pdbqt(smiles, out_path):
    """rdkit ETKDGv3 + MMFF 3D prep; obabel fallback for unusual SMILES."""
    try:
...
def dock_compound(receptor_pdbqt, lig_pdbqt, box_center, timeout=90):
    # Fork child so SIGKILL enforces timeout even against Vina's pure-C loops.
    ...
    if p.is_alive():
        p.kill()
        p.join(timeout=10)   # cap post-SIGKILL wait — D-state processes block forever
        if p.is_alive():
...
def run_bo(model, X_all, y_all, names, start_obs=None, start_round=1, start_best=None):
    ...
    for rnd in range(start_round, N_BO + 1):
        ...
        heartbeat("Phase39_BO_MSH3", rnd, {"best_pkd": best_pkd})
...
def main():
    ...
    log("=" * 65)
    log("  Phase 39 — MSH3 3THW Docking (chain A, rdkit ETKDGv3 + MMFF)")
    log("=" * 65)
    notify("PHASE_START", "[Phase39] MSH3 3THW rdkit ligand prep", data={})
    ...
    results_base = {
        "experiment":    "phase39_msh3_rdkit",
        "receptor":      f"3THW chain {CHAIN_ID} (MSH3 ATPase, ADP-derived box)",
        "ligand_prep":   "rdkit ETKDGv3 + MMFF (numpy<2 forced)",
    ...
    if len(fps) < 20:
        ...
        notify("PHASE_COMPLETE", "[Phase39] Done — insufficient dockings", data={"n_valid": n_valid})
        return
    ...
    out.write_text(json.dumps(results_base, indent=2))
    gsutil_cp(out, GCS_DONE)
    log("Results → GCS phase39_msh3_rdkit/results.json")
    notify("PHASE_COMPLETE", "[Phase39] MSH3 rdkit done",
